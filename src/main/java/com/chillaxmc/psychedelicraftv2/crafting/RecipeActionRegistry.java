...
//                inventoryCrafting.setInventorySlotContents(i, null);
                ...
                {
                    Psychedelicraftv2.logger.error(String.format("Recipe with ID '%s' did not handle crafting", actionID));
                }
            }
            else
            {
                Psychedelicraftv2.logger.error(String.format("Did not find action recipe for ID '%s'", actionID));
            }
        }
