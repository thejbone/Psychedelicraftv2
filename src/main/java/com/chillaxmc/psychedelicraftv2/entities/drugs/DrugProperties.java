...
package com.chillaxmc.psychedelicraftv2.entities.drugs;

import cpw.mods.fml.common.network.ByteBufUtils;
import io.netty.buffer.ByteBuf;
import ivorius.ivtoolkit.network.PartialUpdateHandler;
import com.chillaxmc.psychedelicraftv2.Psychedelicraftv2;
import com.chillaxmc.psychedelicraftv2.client.rendering.IDrugRenderer;
import com.chillaxmc.psychedelicraftv2.entities.PSAccessHelperEntity;
import com.chillaxmc.psychedelicraftv2.network.PSNetworkHelperServer;
...
public class DrugProperties implements IExtendedEntityProperties, PartialUpdateHandler
{
    ...
    public static void initInEntity(Entity entity)
    {
        if (entity instanceof EntityLivingBase)
        {
            ...
            if (drugProperties != null)
                Psychedelicraftv2.proxy.createDrugRenderer(drugProperties);
        }
    }
    ...
    public void addToDrug(String drugName, double effect)
    {
        ...
        {
            Psychedelicraftv2.logger.warn("Tried to add to drug " + drugName);
            return;
        }
    ...
    public void setDrugValue(String drugName, double effect)
    {
        if (!drugs.containsKey(drugName))
        {
            Psychedelicraftv2.logger.warn("Tried to set drug value " + drugName);
            return;
        }
    ...
    public void updateDrugEffects(EntityLivingBase entity)
    {
        ...
        if (entity.worldObj.isRemote)
        {
            ...
            if (delayUntilHeartbeat == 0)
            {
                ...
                if (heartbeatVolume > 0.0f)
                {
                    ...
                    entity.worldObj.playSound(entity.posX, entity.posY, entity.posZ, Psychedelicraftv2.modBase + "heartBeat", heartbeatVolume, speed, false);
                }
            }
            ...
            if (delayUntilBreath == 0)
            {
                ...
                if (breathVolume > 0.0f)
                {
                    ...
                    entity.worldObj.playSound(entity.posX, entity.posY, entity.posZ, Psychedelicraftv2.modBase + "breath", breathVolume, speed * 0.1f + 0.9f + (lastBreathWasIn ? 0.15f : 0.0f), false);
                }
            }
        ...
        if (timeBreathingSmoke > 0)
        {
            ...
            if (timeBreathingSmoke > 10 && entity.worldObj.isRemote)
            {
                ...
                if (random.nextInt(2) == 0)
                {
                    ...
                    Psychedelicraftv2.proxy.spawnColoredParticle(entity, breathSmokeColor, look, s, 1.0f);
                }
                if (random.nextInt(5) == 0)
                {
                    float s = random.nextFloat() * 0.05f + 0.1f;
                    Psychedelicraftv2.proxy.spawnColoredParticle(entity, breathSmokeColor, look, s, 2.5f);
                }
            }
        }
        ...
        if (hasChanges)
        {
            ...
            if (!entity.worldObj.isRemote)
                PSNetworkHelperServer.sendEEPUpdatePacket(entity, EEP_KEY, "DrugData", Psychedelicraftv2.network);
        }
    }
